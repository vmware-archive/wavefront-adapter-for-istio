...
RUN tdnf --refresh install -qy wget tar gzip git && \
...
COPY ./ .
RUN CGO_ENABLED=0 GOOS=linux /usr/local/go/bin/go build -a -installsuffix cgo -v -o bin/wavefront ./wavefront/cmd/ >/dev/null 2>&1
